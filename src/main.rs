...
async fn main() {
    ...
    let requested_session_path = cli
        .session_path
        .as_deref()
        .unwrap_or_else(|| std::path::Path::new(".pi/session.jsonl"));
    let session_path =
    ...
    if cli.continue_session {
        let _ = session_store.continue_most_recent();
    }

    if let Some(from_entry_id) = cli.branch_from {
        session_store
            .branch_from(from_entry_id)
            .await
            .unwrap_or_else(|err| panic!("failed to branch from session entry: {err}"));
    }
