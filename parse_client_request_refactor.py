...
start = content.find("pub fn parse_client_request")
end = content.find("fn as_opt_string", start)
original_fn = content[start:end]

new_fn_body = """pub fn parse_client_request(raw: &str) -> Result<ClientRequest, ProtocolError> {
...
with open("crates/pi-protocol/src/rpc.rs", "w") as f:
    f.write(content.replace(original_fn, new_fn_body))
