...
replacement = r"""
                    Matcher::Fuzzy => {
                        ...
                        // If it's >= 0.72, then dist <= 0.28 * max.
                        // Since dist >= diff, if diff > 0.28 * max, it's impossible.
                        if (diff as f64) > 0.28 * (max_chars as f64) {
                            Vec::new()
                        } else {
                            // Can we avoid allocation? If the string only has ASCII we can optimize further.
                            // But let's check basic early out first.
                            lower_line.clear();
                            for c in line.chars() {
