...
mod tests {
    ...
    fn test_parse_client_request_not_object() {
        ...
        match result.expect_err("Expected error when parsing non-object JSON") {
    ...
    fn test_parse_client_request_invalid_json() {
        ...
        match result.expect_err("Expected error when parsing invalid JSON") {
    ...
    fn test_to_jsonl_value_ready_event() {
        ...
        let parsed: Value =
            serde_json::from_str(&result).expect("Failed to parse ready event JSON to Value");
    ...
    fn test_to_jsonl_value_error_event() {
        ...
        let result = to_jsonl_value(&event);
        let parsed: Value =
            serde_json::from_str(&result).expect("Failed to parse error event JSON to Value");
    ...
    fn test_to_jsonl_value_turn_start_event() {
        ...
        let result = to_jsonl_value(&event);
        let parsed: Value =
            serde_json::from_str(&result).expect("Failed to parse turn start event JSON to Value");
    ...
    fn test_to_jsonl_value_message_update_event() {
        ...
        let parsed: Value = serde_json::from_str(&result)
            .expect("Failed to parse message update event JSON to Value");
