...
fn matches_scope(entry: &IndexedFile, scope: Option<&str>) -> bool {
    scope.as_ref().map_or(true, |scope| scope_is_prefix(&entry.relative_path, scope))
}
...
fn matches_filters(entry: &IndexedFile, filters: &[SearchFilter], query: &str) -> bool {
    ...
    for filter in filters {
        let ext_ok = filter
            .extension
            .as_ref()
            .as_ref().map_or(true, |ext| entry.relative_path.ends_with(&format!(".{ext}")));
        let scope_ok = filter
            .path_prefix
            .as_ref()
            .as_ref().map_or(true, |prefix| scope_is_prefix(&entry.relative_path, prefix));
        if !ext_ok || !scope_ok {
            return false;
        }
    }
